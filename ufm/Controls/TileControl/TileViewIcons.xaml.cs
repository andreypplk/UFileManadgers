...
//        #region Обработка событий мыши для выделения при наведении (обновлено)
...
//            {
//                _hoveredItem = item;
//                // При зажатой левой кнопке сразу выделяем
//                SelectItemOnHover(_hoveredItem);
//            }
//        }
...
//            var point = e.GetCurrentPoint(ItemsGridView);
//            // Сбрасываем hoveredItem только если левая кнопка нажата
//            if (point.Properties.IsLeftButtonPressed)
//            {
//                _hoveredItem = null;
//            }
...
//            var point = e.GetCurrentPoint(ItemsGridView);
//            // ПРОВЕРКА: обрабатываем только если левая кнопка зажата
//            if (!point.Properties.IsLeftButtonPressed)
//            {
//                _hoveredItem = null;
//                return;
...
//                else
//                {
//                    // ЗАКОММЕНТИРОВАНО: Если кликнули на элементе, НЕ начинаем отслеживание движения с зажатой кнопкой
//                    // ItemsGridView.CapturePointer(e.Pointer);

//                    // Вместо этого позволяем обычные клики работать
//                    // Движение с зажатой кнопкой будет обрабатываться в ItemsGridView_PointerMovedForDrag
//                    // без захвата указателя

//                    // Не обрабатываем событие, чтобы позволить стандартной обработке клика
//                    e.Handled = false;
//                }
//            }
//        }
...
//            // Не проверяем CapturePointer, так как не захватываем указатель при клике на элементе
...
//                if (_isMouseMovingWithButton)
//                {
//                    ProcessHoverSelection(currentPosition);
//                }
...
//        private void StartHoverSelection()
//        {
//            // Начинаем выделение при движении с зажатой кнопкой
//            Debug.WriteLine("Начато выделение при движении с зажатой кнопкой");
//        }

//        private void ProcessHoverSelection(Windows.Foundation.Point currentPosition)
//        {
//            // Находим элемент под курсором
//            var elements = VisualTreeHelper.FindElementsInHostCoordinates(currentPosition, ItemsGridView);
//            var element = elements.FirstOrDefault(e => e is FrameworkElement fe && fe.DataContext is ExplorerItemViewModel);

//            var item = FindParentDataContext<ExplorerItemViewModel>(element as FrameworkElement);
...
//        #region Обработка событий мыши для выделения при наведении (исправлено)
...
//            else
//            {
//                // Режим ДВОЙНОГО клика - выделяем и ждем второго клика
//                if (isShiftPressed)
//                {
//                    // Shift+клик - выделение диапазона
//                    if (_shiftSelectionStartItem == null)
...
//                        // (только для режима двойного клика!)
//                        _ = this.DispatcherQueue.EnqueueAsync(() =>
//                        {
...
//        #region Выделение областью мышью - ОПТИМИЗИРОВАННАЯ ВЕРСИЯ
...
//                    int clickedIndex = ItemsGridView.IndexFromContainer(clickedItem);
...
//                // Применяем окончательное выделение к GridView
//                ApplySelectionToGridView();
...
//                    // Выделяем элементы внутри прямоугольника (ОПТИМИЗИРОВАННАЯ версия)
//                    PerformRectangleSelection(_dragStartPoint, currentPoint);
...
//        {
//            // Находим элемент под курсором
//            var item = FindItemAtPoint(currentPosition);
//            if (item != null)
//            {
//                var itemViewModel = item.DataContext as ExplorerItemViewModel;
//                if (itemViewModel != null && itemViewModel != _hoveredItem)
//                {
//                    _hoveredItem = itemViewModel;
//                    SelectItemOnHover(_hoveredItem);
//                }
//            }
//        }
...
//        // ОПТИМИЗИРОВАННОЕ выделение прямоугольником
//        private void PerformRectangleSelection(Vector2 startPoint, Vector2 endPoint)
//        {
//            if (Items.Count == 0) return;
...
//        private void ApplySelectionToGridView()
//        {
//            // Очищаем текущее выделение, если не нажат Ctrl
//            if (!_isCtrlPressed)
//            {
//                ItemsGridView.SelectedItems.Clear();
//            }
...
//            foreach (int index in _tempSelectedIndices)
//            {
//                if (index >= 0 && index < Items.Count)
//                {
