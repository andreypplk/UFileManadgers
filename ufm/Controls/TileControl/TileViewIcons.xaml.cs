...
//            // Добавляем обработчик кликов на ItemsGridView
...
//                    // Не обрабатываем событие, чтобы позволить стандартной обработке клика через ItemsGridView_OnItemClick
//                    e.Handled = false;
//                }
//                else
//                {
...
//            if (ItemsGridView.SelectedItem is ExplorerItemViewModel selectedItem && selectedItem.IsEditing)
//            {
//                Debug.WriteLine($"[TileViewIcons {PanelId}] Cancelling edit mode before opening new item");
//                var container = ItemsGridView.ContainerFromItem(selectedItem) as GridViewItem;
//                if (container?.ContentTemplateRoot is BaseTileControl tileControl && tileControl.IsEditing)
//                {
//                    tileControl.CancelEditing();
//                }
//            }
...
//            try
//            {
//                Debug.WriteLine($"[TileViewIcons {PanelId}] === RenameSelectedItem START ===");

//                if (ItemsGridView.SelectedItem is ExplorerItemViewModel selectedItem)
//                {
//                    Debug.WriteLine($"[TileViewIcons {PanelId}] Selected item: {selectedItem.Name}");
//                    Debug.WriteLine($"[TileViewIcons {PanelId}] IsEditing: {selectedItem.IsEditing}, EditRequested: {selectedItem.EditRequested}");

//                    // Проверяем, не редактируется ли уже этот элемент
//                    if (selectedItem.IsEditing)
//                    {
//                        Debug.WriteLine($"[TileViewIcons {PanelId}] Item is already in edit mode");
//                        return;
//                    }
...
//                        if (container.ContentTemplateRoot is BaseTileControl tileControl)
//                        {
//                            if (tileControl.CanEdit)
//                            {
//                                Debug.WriteLine($"[TileViewIcons {PanelId}] Calling StartEditing on tile control");
//                                tileControl.StartEditing();
//                            }
//                            else
//                            {
//                                Debug.WriteLine($"[TileViewIcons {PanelId}] Tile control doesn't support editing");
//                            }
//                        }
//                        else
//                        {
//                            Debug.WriteLine($"[TileViewIcons {PanelId}] ContentTemplateRoot is not BaseTileControl");
//                        }
//                    }
//                    else
//                    {
//                        Debug.WriteLine($"[TileViewIcons {PanelId}] Container not found, scrolling into view...");
...
//                            container = ItemsGridView.ContainerFromItem(selectedItem) as GridViewItem;
//                            if (container?.ContentTemplateRoot is BaseTileControl tileControl2 && tileControl2.CanEdit)
//                            {
//                                Debug.WriteLine($"[TileViewIcons {PanelId}] Retry successful, calling StartEditing");
//                                tileControl2.StartEditing();
//                            }
...
//                else
//                {
//                    Debug.WriteLine($"[TileViewIcons {PanelId}] No item selected");
//                }

//                Debug.WriteLine($"[TileViewIcons {PanelId}] === RenameSelectedItem END ===");
//            }
//            catch (Exception ex)
//            {
//                Debug.WriteLine($"[TileViewIcons {PanelId}] Error in RenameSelectedItem: {ex.Message}");
//            }
//        }
...
namespace ufm
{
    public sealed partial class TileViewIcons : UserControl, IDisposable, ISupportsIconSize, IRefreshablePanel
    {
        ...
        private void SelectItemOnHover(ExplorerItemViewModel item)
        {
            ...
            // НЕ выделяем элемент ".." при наведении! - ВАЖНОЕ ИСПРАВЛЕНИЕ
            if (item.Name == "..") return;
        ...
        private async void ItemsGridView_OnItemClick(object sender, ItemClickEventArgs e)
        {
            // ЗАЩИТА ОТ БЫСТРЫХ ПОВТОРНЫХ КЛИКОВ - ВАЖНОЕ ИСПРАВЛЕНИЕ
            ...
            // ДОБАВЛЕНА ОТЛАДОЧНАЯ ИНФОРМАЦИЯ КАК В TILELISTVIEW
            Debug.WriteLine($"[{PanelId}] === ItemsGridView_OnItemClick START ===");
            ...
            if (!isSingleClickMode)
            {
                if (_wasClickHandled)
                {
                    ...
                    Debug.WriteLine($"[{PanelId}] === ItemsGridView_OnItemClick END (skipped) ===");
                    return;
                }
                _wasClickHandled = true;
            ...
            Debug.WriteLine($"[{PanelId}] === ItemsGridView_OnItemClick END ===");
        }
        ...
        internal async Task LoadPathContents(string path)
        {
            // Блокируем параллельные вызовы навигации семафором - ВАЖНОЕ ИСПРАВЛЕНИЕ
        ...
        private async Task LoadDrives()
        {
            ...
            try
            {
                ...
                // ИСПРАВЛЕННЫЙ КОД - используем DispatcherQueue как в TileListView
                await this.DispatcherQueue.EnqueueAsync(() =>
                {
                    Items.Clear();
                    foreach (var item in driveItems)
        ...
        private void ItemsGridView_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            foreach (ExplorerItemViewModel removedItem in e.RemovedItems)
            {
                ...
                if (removedItem?.Name == "..")
                {
                    // При удалении элемента ".." из выделения сбрасываем hover-таймер - ИСПРАВЛЕНИЕ
                    StopHoverTimer();
                    _hoveredItem = null;
                }
            }
        ...
        private async Task OpenItemByIndex(int index)
        {
            ...
            // ЗАЩИТА ОТ ПАРАЛЛЕЛЬНЫХ ОБРАБОТОК НАВИГАЦИИ НАЗАД - ВАЖНОЕ ИСПРАВЛЕНИЕ
