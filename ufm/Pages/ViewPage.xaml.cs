...
namespace ufm
{
    public sealed partial class ViewPage : Page
    {
        ...
        private async void ViewPage_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            // --- Снимок: пытаемся восстановить содержимое активной папки из снимка ---
            TryLoadActiveFolderSnapshot();
        ...
        private void ViewPage_Unloaded(object sender, RoutedEventArgs e)
        {
            // --- Снимок: сохраняем содержимое активной папки ---
            SaveActiveFolderSnapshot();
        ...
        #region --- Снимок: сохранение/восстановление содержимого активной папки ---

        private void SaveActiveFolderSnapshot()
        {
            if (_activePanelManager == null || _activeTileViewer == null)
                return;

            string folderPath = _activePanelManager.CurrentPath;
            if (string.IsNullOrEmpty(folderPath) || !Directory.Exists(folderPath))
                return;

            try
            {
                var items = _activeTileViewer.Items?.Select(vm =>
                {
                    bool isDir = vm is ExplorerItemViewModel ? Directory.Exists(vm.FilePath) : false;
                ...
                var folderSnap = new SnapshotNode
                {
                    Path = folderPath,
                    Name = System.IO.Path.GetFileName(folderPath),
                    IsDirectory = true,
                    Children = items ?? new List<SnapshotNode>()
                };

                App.SettingsManager?.SaveSetting("ActiveFolderSnapshot", SerializeSnapshotNode(folderSnap));
            }
            catch { }
        }
        ...
        private void TryLoadActiveFolderSnapshot()
        {
            var dict = App.SettingsManager?.GetSetting<Dictionary<string, object>>("ActiveFolderSnapshot");
            if (dict == null)
                return;

            var folderSnap = DeserializeSnapshotNode(dict);
            if (folderSnap == null || string.IsNullOrEmpty(folderSnap.Path) || !Directory.Exists(folderSnap.Path))
                return;

            if (_activePanelManager?.CurrentPath != folderSnap.Path)
                return;

            if (_activeTileViewer == null)
            {
                SetViewMode(_activePanelManager.State.ViewMode);
            }

            _activeTileViewer?.LoadFromSnapshot(folderSnap);
        }
        ...
        private SnapshotNode DeserializeSnapshotNode(Dictionary<string, object> dict)
        {
            var snap = new SnapshotNode
            {
                ...
                IsDirectory = (bool)dict["IsDirectory"],
