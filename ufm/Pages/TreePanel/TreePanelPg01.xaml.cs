...
namespace ufm
{
    public sealed partial class TreePanelPg01 : Page, IDisposable
    {
        ...
        // Сохранённое состояние раскрытых узлов и выделенного пути
        ...
        public bool ExpandedTreeSelectedSetting
        {
            get
            {
                try
                {
                    var value = App.SettingsManager?.GetSetting<bool>("ExpandedTreeSelected", true) ?? true;
                    Debug.WriteLine($"[TreePanelPg01] ExpandedTreeSelectedSetting = {value}");
                    return value;
                }
                catch (Exception ex)
                {
        ...
        public bool ExpanderNodesSFStartsSetting
        {
            get
            {
                try
                {
                    var value = App.SettingsManager?.GetSetting<bool>("ExpanderNodesSFStarts", true) ?? true;
                    Debug.WriteLine($"[TreePanelPg01] ExpanderNodesSFStartsSetting = {value}");
                    return value;
                }
                catch (Exception ex)
                {
        ...
        public bool ExpanderNodesMyPcStartsSetting
        {
            get
            {
                try
                {
                    var value = App.SettingsManager?.GetSetting<bool>("ExpanderNodesMyPcStarts", true) ?? true;
                    Debug.WriteLine($"[TreePanelPg01] ExpanderNodesMyPcStartsSetting = {value}");
                    return value;
                }
                catch (Exception ex)
                {
        ...
        //public TreePanelPg01()
        //{
        //    Debug.WriteLine($"[TreePanelPg01] Constructor START");
        //    this.InitializeComponent();
        ...
        //    // Подписки на события разворачивания/сворачивания для основного дерева
        //    treeView.Expanding += (s, e) => AddExpandedPath(e.Node);
        //    // НЕ ПОДПИСЫВАЕМСЯ НА COLLAPSED ДЛЯ УДАЛЕНИЯ - сохраняем состояние!
        //    // treeView.Collapsed += (s, e) => RemoveExpandedPath(e.Node); // <-- УДАЛЕНО!
        ...
        //    // Подписки на события разворачивания/сворачивания для дерева специальных папок
        //    treeViewSpF.Expanding += (s, e) => AddExpandedPath(e.Node);
        //    // НЕ ПОДПИСЫВАЕМСЯ НА COLLAPSED ДЛЯ УДАЛЕНИЯ - сохраняем состояние!
        //    // treeViewSpF.Collapsed += (s, e) => RemoveExpandedPath(e.Node); // <-- УДАЛЕНО!

        //    Debug.WriteLine($"[TreePanelPg01] Constructor END");
        //}
        // В конструкторе добавьте загрузку начальных настроек
        public TreePanelPg01()
        {
            this.InitializeComponent();
            ...
            // Загружаем сохранённые раскрытые узлы из настроек для новой вкладки
        ...
        // Добавьте этот метод для загрузки начального состояния из настроек
        private void LoadDefaultExpandedState()
        {
            try
            {
                // Загружаем сохранённые пути из настроек (если есть)
                ...
                if (savedPaths != null && savedPaths.Count > 0)
                {
                    ...
                    Debug.WriteLine($"[TreePanelPg01] LoadDefaultExpandedState: loaded {_savedExpandedPaths.Count} paths");
                }
                else
                {
                    // Если нет сохранённых путей, используем настройку разворачивания корневого узла
                    if (ExpanderNodesMyPcStartsSetting)
                    {
                        _savedExpandedPaths.Add("MyComputer");
                        Debug.WriteLine($"[TreePanelPg01] LoadDefaultExpandedState: added MyComputer from setting");
                    }
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[TreePanelPg01] LoadDefaultExpandedState error: {ex.Message}");
                if (ExpanderNodesMyPcStartsSetting)
                    _savedExpandedPaths.Add("MyComputer");
            }
        }

        // В TreePanelPg01_Unloaded сохраняем состояние в настройки
        private void TreePanelPg01_Unloaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] Unloaded START, saving state, expanded paths={_expandedPaths.Count}");
            ...
            try
            {
                ...
                Debug.WriteLine($"[TreePanelPg01] Unloaded: saved {pathsList.Count} paths to settings");
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[TreePanelPg01] Unloaded save error: {ex.Message}");
            }

            Debug.WriteLine($"[TreePanelPg01] Unloaded END, savedExpandedPaths={_savedExpandedPaths.Count}, savedSelectedPath={_savedSelectedPath}");
        }
        public void Dispose()
        {
            Debug.WriteLine($"[TreePanelPg01] Dispose START");
            if (!_disposed)
            {
                try
                {
            ...
            Debug.WriteLine($"[TreePanelPg01] Dispose END");
        }
        ...
        private void AddExpandedPath(TreeViewNode node)
        {
            if (node.Content is ExplorerItemViewModel item && !string.IsNullOrEmpty(item.FilePath))
            {
                ...
                Debug.WriteLine($"[TreePanelPg01] AddExpandedPath: {item.FilePath}, total={_expandedPaths.Count}");
            }
        }
        ...
        private void RestoreExpandedState(TreeView tree, IList<TreeViewNode> nodes, string treeName)
        {
            ...
            Debug.WriteLine($"[TreePanelPg01] RestoreExpandedState for {treeName}, checking {nodes.Count} root nodes, saved paths={_expandedPaths.Count}");
            ...
            foreach (var node in nodes)
            {
                if (node.Content is ExplorerItemViewModel item)
                {
                    ...
                    Debug.WriteLine($"[TreePanelPg01]   Node: {item.FilePath}, shouldExpand={shouldExpand}, IsExpanded={node.IsExpanded}");

                    if (shouldExpand && !node.IsExpanded)
                    {
                        Debug.WriteLine($"[TreePanelPg01]   Expanding node: {item.FilePath}");
                        tree.Expand(node);
                    }
                }
        ...
        //    // Сохраняем раскрытые пути
        //    _savedExpandedPaths.Clear();
        //    foreach (var path in _expandedPaths)
        //        _savedExpandedPaths.Add(path);

        //    // Сохраняем текущий выделенный путь
        //    _savedSelectedPath = _currentSelectedItemPath;
        ...
        private void TreePanelPg01_Loaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] Loaded START, _isFirstLoad={_isFirstLoad}");

            // Восстанавливаем сохранённые раскрытые пути
            _expandedPaths.Clear();
            foreach (var path in _savedExpandedPaths)
                _expandedPaths.Add(path);

            Debug.WriteLine($"[TreePanelPg01] Loaded: restored expandedPaths count={_expandedPaths.Count}");

            if (App.SettingsManager != null)
            {
                _selectedSize = App.SettingsManager.GetSetting<string>("SelectedSizeIconTreeView");
                Debug.WriteLine($"[TreePanelPg01] Loaded: _selectedSize from SettingsManager={_selectedSize}");
            }
            ...
            if (string.IsNullOrEmpty(_selectedSize))
            {
                ...
                {
                    _selectedSize = localSettings["SelectedSizeIconTreeView"]?.ToString();
                    Debug.WriteLine($"[TreePanelPg01] Loaded: _selectedSize from LocalSettings={_selectedSize}");
                }
            }
            ...
            // Если это не первая загрузка (переключение вкладок), восстанавливаем состояние
            if (!_isFirstLoad)
            {
                Debug.WriteLine($"[TreePanelPg01] Loaded: NOT first load, scheduling restore");
                DispatcherQueue.TryEnqueue(async () =>
                {
                    await Task.Delay(300); // Даём время на полную загрузку дерева
                    Debug.WriteLine($"[TreePanelPg01] Loaded: Starting restore, expandedPaths count={_expandedPaths.Count}");
                    ...
                    {
                        Debug.WriteLine($"[TreePanelPg01] Loaded: Restoring selection to {_savedSelectedPath}");
            ...
            {
                Debug.WriteLine($"[TreePanelPg01] Loaded: First load, skipping restore");
                _isFirstLoad = false;
            }

            Debug.WriteLine($"[TreePanelPg01] Loaded END");
        }
        ...
        private void SetSelectedRadioButton(string selectedSize)
        {
            Debug.WriteLine($"[TreePanelPg01] SetSelectedRadioButton: {selectedSize}");
            ExtraSmallSizeRadioButton.IsChecked = false;
            SmallSizeRadioButton.IsChecked = false;
            MediumSizeRadioButton.IsChecked = false;
            LargeSizeRadioButton.IsChecked = false;
        ...
        private void SizeRadioButton_Checked(object sender, RoutedEventArgs e)
        {
            if (sender is RadioButton radioButton && radioButton.Tag is string selectedSize)
            {
                Debug.WriteLine($"[TreePanelPg01] SizeRadioButton_Checked: {selectedSize}");
                _selectedSize = selectedSize;
                ...
                if (App.SettingsManager != null)
                {
                    try
                    {
                        App.SettingsManager.SaveSetting("SelectedSizeIconTreeView", _selectedSize);
                        saved = true;
                        Debug.WriteLine($"[TreePanelPg01] Size saved via SettingsManager");
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine($"Ошибка SettingsManager: {ex.Message}");
                ...
                if (!saved)
                {
                    try
                    {
                        var localSettings = ApplicationData.Current.LocalSettings.Values;
                        localSettings["SelectedSizeIconTreeView"] = _selectedSize;
                        Debug.WriteLine($"[TreePanelPg01] Size saved via LocalSettings");
                    }
                    catch (Exception ex)
                    {
                        Debug.WriteLine($"Ошибка LocalSettings: {ex.Message}");
        ...
        private async void TreeView_OnLoaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeView_OnLoaded START, ExpanderNodesMyPcStartsSetting={ExpanderNodesMyPcStartsSetting}");
            ...
            treeView.RootNodes.Add(myComputerNode);
            Debug.WriteLine($"[TreePanelPg01] TreeView_OnLoaded: myComputerNode created, IsExpanded={myComputerNode.IsExpanded}");

            DispatcherQueue.TryEnqueue(async () =>
            {
                Debug.WriteLine($"[TreePanelPg01] TreeView_OnLoaded: DispatcherQueue executing");
                UpdateTileSize(myComputerNode);

                if (ExpanderNodesMyPcStartsSetting)
                {
                    Debug.WriteLine($"[TreePanelPg01] TreeView_OnLoaded: Loading drives");
                    await LoadDrivesSync(myComputerNode);
                    await PreloadFirstLevelAsync(myComputerNode);

                    // Принудительно раскрываем узел
                    myComputerNode.IsExpanded = true;
                    treeView.Expand(myComputerNode);
                    Debug.WriteLine("Основной TreeView: узел 'Мой компьютер' раскрыт (настройка разрешает)");
                }
                ...
                Debug.WriteLine($"[TreePanelPg01] TreeView_OnLoaded: After expand, myComputerNode.IsExpanded={myComputerNode.IsExpanded}");
            });

            Debug.WriteLine($"[TreePanelPg01] TreeView_OnLoaded END");
        }

        private async Task PreloadFirstLevelAsync(TreeViewNode myComputerNode)
        {
            Debug.WriteLine($"[TreePanelPg01] PreloadFirstLevelAsync START, children count={myComputerNode.Children.Count}");
            foreach (var driveNode in myComputerNode.Children)
            {
                if (driveNode.Content is ExplorerItemViewModel driveItem && Directory.Exists(driveItem.FilePath))
                {
                    try
                    {
                        Debug.WriteLine($"[TreePanelPg01] PreloadFirstLevelAsync: Loading for drive {driveItem.FilePath}");
                        var firstLevelItems = await _fileSystemService.LoadFoldersOnlyAsync(driveItem.FilePath, _history);
                        foreach (var item in firstLevelItems.Take(3))
                        {
                            item.IsTreeViewNode = true;
                            ...
                            driveNode.Children.Add(node);
                            Debug.WriteLine($"[TreePanelPg01] PreloadFirstLevelAsync: Added {item.FilePath}");
                        }
                    }
                    catch (Exception ex)
                    {
            ...
            Debug.WriteLine($"[TreePanelPg01] PreloadFirstLevelAsync END");
        }

        private void ExpandMyComputerNode(TreeViewNode myComputerNode)
        {
            Debug.WriteLine($"[TreePanelPg01] ExpandMyComputerNode START, HasUnrealizedChildren={myComputerNode.HasUnrealizedChildren}, Children.Count={myComputerNode.Children.Count}");
            try
            {
                if (myComputerNode.HasUnrealizedChildren || myComputerNode.Children.Count == 0)
                {
            ...
            UpdateTileSize(myComputerNode);
            Debug.WriteLine($"[TreePanelPg01] ExpandMyComputerNode END, IsExpanded={myComputerNode.IsExpanded}");
        }

        private async void TreeView_OnExpanding(TreeView sender, TreeViewExpandingEventArgs args)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeView_OnExpanding START");
            if (args.Node.Content is not ExplorerItemViewModel treeItem)
                return;

            Debug.WriteLine($"[TreePanelPg01] TreeView_OnExpanding: Expanding {treeItem.FilePath}");
            try
            {
                if (treeItem.Name == "Мой Компьютер" && (args.Node.HasUnrealizedChildren || args.Node.Children.Count == 0))
                {
            ...
            Debug.WriteLine($"[TreePanelPg01] TreeView_OnExpanding END");
        }

        private void TreeView_OnDoubleTapped(object sender, DoubleTappedRoutedEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeView_OnDoubleTapped");
            if (treeView.SelectedNode == null) return;
        ...
        private void TreeView_OnCollapsed(TreeView sender, TreeViewCollapsedEventArgs args)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeView_OnCollapsed");
            // НЕ УДАЛЯЕМ путь из _expandedPaths - сохраняем состояние для переключения вкладок
            if (args.Node.HasChildren && args.Node.Children.Count > 0)
            {
                args.Node.Children.Clear();
                args.Node.HasUnrealizedChildren = true;
        ...
        private void TreeViewSpF_OnLoaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_OnLoaded START, _isTreeViewSpFInitialized={_isTreeViewSpFInitialized}");
            if (_isTreeViewSpFInitialized) return;
            ...
            _isTreeViewSpFInitialized = true;
            UpdateAllTiles();
            Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_OnLoaded: specialFoldersNode created, IsExpanded={specialFoldersNode.IsExpanded}");

            DispatcherQueue.TryEnqueue(async () =>
            {
                Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_OnLoaded: DispatcherQueue executing");
                await LoadHomeContentsAsync(specialFoldersNode);
                await PreloadFirstLevelForSpFAsync(specialFoldersNode);
                ...
                Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_OnLoaded: After expand, IsExpanded={specialFoldersNode.IsExpanded}");
            });
            Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_OnLoaded END");
        }

        private async Task PreloadFirstLevelForSpFAsync(TreeViewNode specialFoldersNode)
        {
            Debug.WriteLine($"[TreePanelPg01] PreloadFirstLevelForSpFAsync START");
            foreach (var systemFolderNode in specialFoldersNode.Children)
            {
                if (systemFolderNode.Content is ExplorerItemViewModel folderItem && Directory.Exists(folderItem.FilePath))
                {
            ...
            Debug.WriteLine($"[TreePanelPg01] PreloadFirstLevelForSpFAsync END");
        }

        private void ExpandSpecialFoldersNode(TreeViewNode specialFoldersNode)
        {
            Debug.WriteLine($"[TreePanelPg01] ExpandSpecialFoldersNode START");
            try
            {
                if (specialFoldersNode.HasUnrealizedChildren || specialFoldersNode.Children.Count == 0)
                {
            ...
            Debug.WriteLine($"[TreePanelPg01] ExpandSpecialFoldersNode END");
        }

        private async void TreeViewSpF_OnExpanding(TreeView sender, TreeViewExpandingEventArgs args)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_OnExpanding START");
            if (_isLoadingSpF || args.Node.Content is not ExplorerItemViewModel treeItem)
                return;
            ...
            UpdateAllTiles();
            Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_OnExpanding END");
        }

        private async Task LoadHomeContentsAsync(TreeViewNode parentNode)
        {
            Debug.WriteLine($"[TreePanelPg01] LoadHomeContentsAsync START");
            try
            {
                parentNode.Children.Clear();
                Debug.WriteLine("LoadHomeContentsAsync начал выполнение");
                ...
                foreach (var item in homeItems)
                {
                    ...
                    parentNode.Children.Add(node);
                    UpdateTileSizeSpF(node);
                    Debug.WriteLine($"[TreePanelPg01] LoadHomeContentsAsync: Added {item.FilePath}");
                }
            ...
            Debug.WriteLine($"[TreePanelPg01] LoadHomeContentsAsync END");
        }

        private void TreeViewSpF_OnCollapsed(TreeView sender, TreeViewCollapsedEventArgs args)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_OnCollapsed");
            // НЕ УДАЛЯЕМ путь из _expandedPaths - сохраняем состояние для переключения вкладок
            if (args.Node.HasChildren && args.Node.Children.Count > 0 &&
                args.Node.Content is ExplorerItemViewModel item &&
                item.FilePath != "SpecialFolders")
        ...
        private void TreeViewSpF_OnDoubleTapped(object sender, DoubleTappedRoutedEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_OnDoubleTapped");
            if (treeViewSpF.SelectedNode == null) return;
        ...
        private async Task LoadDrivesSync(TreeViewNode parentNode)
        {
            Debug.WriteLine($"[TreePanelPg01] LoadDrivesSync START");
            try
            {
                var driveItems = await _fileSystemService.LoadDrivesTree(_history);
                Debug.WriteLine($"[TreePanelPg01] LoadDrivesSync: Loaded {driveItems.Count} drives");
                foreach (var driveItem in driveItems)
                {
                    driveItem.IsTreeViewNode = true;
                    var driveNode = new TreeViewNode
                    ...
                    parentNode.Children.Add(driveNode);
                    Debug.WriteLine($"[TreePanelPg01] LoadDrivesSync: Added drive {driveItem.FilePath}");
                }
            }
            catch (Exception ex)
            {
            ...
            Debug.WriteLine($"[TreePanelPg01] LoadDrivesSync END, children count={parentNode.Children.Count}");
        }

        private async Task LoadSubfoldersAsync(TreeViewNode parentNode, string folderPath)
        {
            Debug.WriteLine($"[TreePanelPg01] LoadSubfoldersAsync START for {folderPath}");
            try
            {
                var folderItems = await _fileSystemService.LoadSubfoldersForTreeViewAsync(folderPath, _history);
                Debug.WriteLine($"[TreePanelPg01] LoadSubfoldersAsync: Loaded {folderItems.Count} items");
                foreach (var item in folderItems)
                {
                    item.IsTreeViewNode = true;
                    var node = new TreeViewNode
            ...
            Debug.WriteLine($"[TreePanelPg01] LoadSubfoldersAsync END");
        }

        private async Task LoadSubfoldersForSpFAsync(TreeViewNode parentNode, string folderPath)
        {
            Debug.WriteLine($"[TreePanelPg01] LoadSubfoldersForSpFAsync START for {folderPath}");
            try
            {
                var folderItems = await _fileSystemService.LoadFoldersOnlyAsync(folderPath, _historySpF);
                Debug.WriteLine($"[TreePanelPg01] LoadSubfoldersForSpFAsync: Loaded {folderItems.Count} items");
                foreach (var item in folderItems)
                {
                    item.IsTreeViewNode = true;
                    var node = new TreeViewNode
            ...
            Debug.WriteLine($"[TreePanelPg01] LoadSubfoldersForSpFAsync END");
        }
        ...
        private async void UpdateTreeViewSelection(string panelId, string path)
        {
            Debug.WriteLine($"[TreePanelPg01] UpdateTreeViewSelection START: panelId={panelId}, path={path}");
            try
            {
                TreeView targetTreeView = panelId == "MainTree" ? treeView : treeViewSpF;
                if (targetTreeView == null || string.IsNullOrEmpty(path))
                {
                    Debug.WriteLine($"[TreePanelPg01] UpdateTreeViewSelection: targetTreeView null or path empty");
                    return;
                }
        ...
        private async Task EnsureMyComputerExpanded(TreeViewNode myComputerNode)
        {
            Debug.WriteLine($"[TreePanelPg01] EnsureMyComputerExpanded START");
            if (myComputerNode?.Content is ExplorerItemViewModel item && item.FilePath == "MyComputer")
            {
                if (!myComputerNode.IsExpanded)
                {
            ...
            Debug.WriteLine($"[TreePanelPg01] EnsureMyComputerExpanded END");
        }

        private async Task ExpandNode(TreeViewNode node, string path, string panelId)
        {
            Debug.WriteLine($"[TreePanelPg01] ExpandNode START: path={path}, panelId={panelId}");
            try
            {
                if (panelId == "MainTree")
                {
            ...
            Debug.WriteLine($"[TreePanelPg01] ExpandNode END");
        }
        ...
        private void OnNavigationChanged(object sender, NavigationEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] OnNavigationChanged: PanelId={e.PanelId}, Path={e.Path}");
            if (e.PanelId == "MainTree" || e.PanelId == "SpFTree")
                _activePanelId = e.PanelId;
            else
                _activePanelId ??= "MainTree";
        ...
        private void TreeViewSpF_SelectionChanged(TreeView sender, TreeViewSelectionChangedEventArgs args)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeViewSpF_SelectionChanged");
            if (_isNavigationChangingSelection) return;
            if (treeViewSpF.SelectedNode?.Content is ExplorerItemViewModel selectedItem)
            {
        ...
        private void TreeView_SelectionChanged(TreeView sender, TreeViewSelectionChangedEventArgs args)
        {
            Debug.WriteLine($"[TreePanelPg01] TreeView_SelectionChanged");
            if (_isNavigationChangingSelection) return;
            if (treeView.SelectedNode?.Content is ExplorerItemViewModel selectedItem)
            {
                if (_currentSelectedItemPath == selectedItem.FilePath) return;
        ...
        private void BackButton_Click(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] BackButton_Click");
            string activePanel = GetActivePanel();
            if (_navigationManager.CanGoBack(activePanel))
                _navigationManager.GoBack(activePanel);
        }
        ...
        private void ForwardButton_Click(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] ForwardButton_Click");
            string activePanel = GetActivePanel();
            if (_navigationManager.CanGoForward(activePanel))
                _navigationManager.GoForward(activePanel);
        }
        ...
        private void UpButton_Click(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"[TreePanelPg01] UpButton_Click");
            try
            {
                if (!string.IsNullOrEmpty(_currentSelectedItemPath) && Directory.Exists(_currentSelectedItemPath))
                {
