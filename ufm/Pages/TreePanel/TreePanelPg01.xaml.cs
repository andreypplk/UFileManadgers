...
namespace ufm
{
    public sealed partial class TreePanelPg01 : Page, IDisposable, IDropTarget
    {
        ...
        {
            _modifierKeyService.UpdateKeyStateFromCore();
            var targetFolder = GetTargetFolderUnderPointer(e, treeView);
            if (!string.IsNullOrEmpty(targetFolder))
            {
                _currentTargetFolder = targetFolder;
                _currentSelectedItemPath = targetFolder; // для совместимости
            }
            _dragDropService.OnDragOver(e, targetFolder ?? _currentSelectedItemPath, true);
            e.Handled = true;
        }
        ...
        private async void TreeView_Drop(object sender, DragEventArgs e)
        {
            ...
            await RefreshSourceNodesAsync(_lastDragSourcePaths);
            _lastDragSourcePaths.Clear();
        ...
        {
            _modifierKeyService.UpdateKeyStateFromCore();
            var targetFolder = GetTargetFolderUnderPointer(e, treeViewSpF);
            if (!string.IsNullOrEmpty(targetFolder))
            {
                _currentTargetFolder = targetFolder;
                _currentSelectedItemPath = targetFolder;
            }
            _dragDropService.OnDragOver(e, targetFolder ?? _currentSelectedItemPath, true);
            e.Handled = true;
        }
        ...
        {
            // Определяем позицию курсора относительно дерева
            var point = e.GetPosition(tree);
            // Ищем элемент TreeViewItem под курсором
            var elements = VisualTreeHelper.FindElementsInHostCoordinates(point, tree);
            foreach (var element in elements)
            {
                if (element is TreeViewItem tvi && tvi.DataContext is ExplorerItemViewModel item)
                {
                    if (!string.IsNullOrEmpty(item.FilePath) && Directory.Exists(item.FilePath))
                        return item.FilePath;
                }
            }
            // fallback: если не нашли, возвращаем текущий выбранный элемент (может быть null)
            return _currentSelectedItemPath;
        }
