...
namespace ufm
{
    public sealed partial class TreePanelPg01 : Page, IDisposable, IDropTarget
    {
        ...
        public static TreePanelPg01 Instance { get; private set; }
        private ModifierKeyService _modifierKeyService = new ModifierKeyService();
        private string _prefsize = "Tree";                     // префикс режима отображения (по умолчанию "Icons")
        private string _selectedSize = "Tree Medium";          // полный ключ размера, например "Icons Medium"
        private string _activePanelId = "MainTree";
        private readonly FileSystemService _fileSystemService;
        private readonly NavigationManager _navigationManager;
        private readonly DirectoryHistory _history;
        ...
        private bool _isUpdatingSize = false;   // флаг для предотвращения рекурсии при программной установке RadioButton
        ...
        /// <summary>
        /// Возвращает только размерную часть текущего полного ключа (например, "Medium").
        /// Используется для установки RadioButton.
        /// </summary>
        ...
        //    // Загружаем сохранённый размер (полный ключ)
        ...
        //    // Если в сохранённом значении нет префикса (например, просто "Medium"),
        //    // добавляем префикс по умолчанию "Icons "
        //    if (!string.IsNullOrEmpty(savedSize) && !savedSize.Contains(" "))
        //    {
        //        savedSize = $"{_prefsize} {savedSize}";
        //    }

        //    if (!string.IsNullOrEmpty(savedSize))
        //    {
        //        _selectedSize = savedSize;
        //        // Извлекаем префикс, если он есть
        //        string[] parts = _selectedSize.Split(' ', 2);
        //        if (parts.Length == 2)
        //            _prefsize = parts[0];
        //    }
        //    else
        //    {
        //        _selectedSize = $"{_prefsize} Medium";
        //    }
        ...
        //    // Извлекаем только размерную часть, игнорируя любой префикс
        //    string sizePart = SizeHelper.ExtractSizePartFromFullKey(savedSize ?? "");

        //    // Для дерева всегда используется префикс "Tree"
        //    _prefsize = "Tree";
        //    _selectedSize = $"{_prefsize} {sizePart}";

        //    // Перезаписываем настройку правильным ключом, чтобы стереть ошибочные префиксы
        //    try
        //    {
        ...
        /// <summary>
        /// Извлекает из полного ключа (например "Icons Medium") только размерную часть ("Medium").
        /// Аналогично методу из ViewPage.
        /// </summary>
        private string ExtractSizePartFromFullKey(string fullSizeKey)
        {
            ...
            // Проверяем составные размеры "Below Medium", "Above Medium"
            if (parts.Length >= 3)
            {
            ...
            // Проверяем "Extra Small", "Extra Large"
            if (parts.Length >= 2)
            {
        ...
        private void SetSelectedRadioButton(string fullSizeKey)
        {
            ...
            // Снимаем выделение со всех кнопок, не вызывая события
        ...
        private void SizeRadioButton_Checked(object sender, RoutedEventArgs e)
        {
            if (_isUpdatingSize) return;   // Игнорируем, если изменение вызвано программно

            if (sender is RadioButton radioButton && radioButton.Tag is string sizeTag && radioButton.IsChecked == true)
            {
                // Формируем полный ключ с текущим префиксом
        ...
        private void UpdateTileSize(TreeViewNode node)
        {
            ...
            var container = treeView?.ContainerFromNode(node) as TreeViewItem;
            var tile = container?.ContentTemplateRoot as BaseTileControl;
            // Передаём полный ключ размера (например, "Icons Medium")
            if (tile != null) tile.UpdateSize(_selectedSize);
            if (node.IsExpanded)
                foreach (var childNode in node.Children) UpdateTileSize(childNode);
        }
