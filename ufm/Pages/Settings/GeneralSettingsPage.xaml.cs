...
namespace ufm
{
    public sealed partial class GeneralSettingsPage : Page
    {
        ...
        // Исправленный метод с правильным параметром
        ...
        private void LoadCurrentBackdropType()
        {
            try
            {
                ...
                // Получаем текущий BackdropType через свойство MainWindow
                var currentBackdrop = _mainWindow.CurrentBackdropType;

                // Проверяем, что значение есть в enum
                if (!Enum.IsDefined(typeof(BackdropManager.BackdropType), currentBackdrop))
                {
        ...
        private void BackdropTypeComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            try
            {
                ...
                if (!Enum.IsDefined(typeof(BackdropManager.BackdropType), selectedIndex))
                {
                    Debug.WriteLine($"Invalid backdrop index: {selectedIndex}");
                    return;
                }

                // Конвертируем индекс в BackdropManager.BackdropType
                var backdropType = (BackdropManager.BackdropType)selectedIndex;
